...
```bash
curl https://<your-facilitator>/supported
```
...
From this response: `signers["canton:*"][0]` is your `facilitatorParty`. Get the `synchronizerId` from your Canton Scan API or validator config.

## Step 3 — Install

```bash
# Express
npm i @canton-network/x402-canton-core @canton-network/x402-canton-express

# Next.js App Router
npm i @canton-network/x402-canton-core @canton-network/x402-canton-next
```

## Step 4 — Define PaymentRequirements

```typescript
import type { PaymentRequirements } from "@canton-network/x402-canton-core";

const paymentReq: PaymentRequirements = {
  scheme: "exact-canton",
  network: "canton:mainnet",
  amount: "1.0000000000",           // Daml Decimal string, 10 decimal places
  asset: "canton-coin",
  payTo: "<YOUR_PARTY>::1220...",   // your Canton party id — CC lands here
  maxTimeoutSeconds: 120,
  extra: {
    transferMethod: "external-party-amulet-rules",
    facilitatorParty: "<FACILITATOR_PARTY>::1220...",
...
import { cantonPaymentMiddleware } from "@canton-network/x402-canton-express";
...
import { withCantonPayment } from "@canton-network/x402-canton-next";
